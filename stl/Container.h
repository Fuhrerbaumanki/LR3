...
public:
  ...
  explicit UnorderedMap(int p_buckets)
      : m_load_factor(0.), m_max_load_factor(1.) {
    if (p_buckets < 8) {
      m_buckets = 8;
    } else {
      m_buckets = p_buckets;
  ...
  UnorderedMap(const UnorderedMap &p_other)
      : m_buckets(p_other.m_buckets), m_load_factor(p_other.m_load_factor),
  ...
  UnorderedMap(const std::initializer_list<PairType> &p_list)
      : UnorderedMap(p_list.size()) {
    for (auto &elem : p_list) {
      Insert(elem);
    }
  };

  UnorderedMap(UnorderedMap &&p_other)
      : m_buckets(p_other.m_buckets), m_load_factor(p_other.m_load_factor),
        m_max_load_factor(p_other.m_max_load_factor),
        m_hash_obj(p_other.m_hash_obj), m_table(p_other.m_table) {
    m_chains = move(p_other.m_chains);
    p_other.m_buckets = 8;         // .. p_other       move,
    p_other.m_max_load_factor = 1; //       ,
    p_other.m_load_factor = 0;     //     ,      move
  ...
  iterator begin() { return iterator(m_chains.GetHead()); }
  ...
  const iterator Insert(const PairType &p_pair) {
    ...
    if ((current_elements + 1) / static_cast<double>(m_buckets) >=
        m_max_load_factor) {
  ...
  */
  _DataType &operator[](const _KeyType &p_key) {
    ...
    return m_chains.GetReferenceToElement(m_table[bucket_number], p_key,
                                          bucket_number);
  }
  ...
  double MaxLoadFactor() const { return m_max_load_factor; }
  ...
  bool Empty() const { return !m_chains.Size(); }

  int Size() const { return m_chains.Size(); }
  ...
  int GetBucketCount() const { return m_buckets; }
