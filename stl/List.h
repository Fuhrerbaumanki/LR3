...
template <typename _KeyType, typename _DataType, typename _Hash> class Chain {
  using PairType = std::pair<const _KeyType, _DataType>;
...
public:
  ...
  iterator Insert(const PairType &p_pair, NodeType *&p_node_ptr,
                  int p_bucket_number) {
    if (p_node_ptr) {
  ...
  const _DataType &At(NodeType *p_node_ptr, const _KeyType &p_key,
                      int p_bucket_number) const {
    if (p_node_ptr) {
      NodeType *cur_ptr = p_node_ptr;
      while (cur_ptr && cur_ptr->m_bucket_number == p_bucket_number) {
    ...
    throw ItemNotFoundError("ItemNotFoundError: element with such key is not "
                            "present in the container.");
  }

  _DataType &GetReferenceToElement(NodeType *p_node_ptr, const _KeyType &p_key,
                                   int p_bucket_number) {
    if (p_node_ptr) {
      NodeType *cur_ptr = p_node_ptr;
      NodeType *prev_ptr = nullptr;
      ...
      prev_ptr->m_next = new NodeType(PairType(p_key, _DataType()),
                                      p_bucket_number, prev_ptr, cur_ptr);
      if (cur_ptr) {
    ...
    m_head = new NodeType(PairType(p_key, _DataType()), p_bucket_number,
                          nullptr, m_head);
    if (m_head->m_next) {
      m_head->m_next->m_prev = m_head;
    }
  ...
  void Erase(NodeType *&p_node_ptr, const _KeyType &p_key,
             int p_bucket_number) {
    if (p_node_ptr) {
      NodeType *cur_ptr = p_node_ptr;
      while (cur_ptr && cur_ptr->m_bucket_number == p_bucket_number) {
        if (cur_ptr->m_pair.first == p_key) {
          ...
          if (cur_ptr == p_node_ptr) {
            if (next_ptr == nullptr ||
                next_ptr->m_bucket_number != p_bucket_number) {
              p_node_ptr = nullptr;
            } else {
              p_node_ptr = next_ptr;
            }
  ...
  iterator Erase(const iterator &p_iter, NodeType *&p_node_ptr,
                 int p_bucket_number) {
    if (p_iter == nullptr) {
      throw IteratorError(
